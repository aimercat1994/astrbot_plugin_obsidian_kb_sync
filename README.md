...
- **同步进度条**：同步时实时显示进度（阶段、已处理/总数、百分比），SSE 推送
- **同步历史**：顶栏「📋 历史」按钮，查看最近 50 条同步记录（类型、状态、统计、耗时）
- **Toast 通知**：操作成功/失败反馈
...
**📊 同步进度 + 历史**
- 同步进度条：SSE 实时推送，显示阶段/进度/百分比
- 同步历史：最近 50 条记录，含类型、状态、统计、耗时
- 后端新增 2 个 API 端点（SSE 进度 + 历史查询）
