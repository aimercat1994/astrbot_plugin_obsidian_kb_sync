...
### 📎 嵌入语法支持
- **`![[文档名]]`**：嵌入其他文档，显示为可点击的引用卡片
- **`![[图片.png]]`**：嵌入图片，支持 Lightbox 放大查看
- 嵌入卡片带图标、标题和提示文字，hover 时有视觉反馈

### 🔍 搜索高亮
- **搜索按钮**：顶栏新增「🔍 搜索」按钮，点击显示搜索栏
- **实时高亮**：输入关键词时立即在预览内容中高亮所有匹配项
- **导航功能**：支持上一个/下一个（↑/↓）按钮和键盘导航
- **匹配计数**：显示当前匹配位置和总匹配数（如 3/15）
- **自动滚动**：切换匹配项时自动滚动到对应位置

### 🎨 样式增强
- 嵌入文档卡片：带背景色、边框和 hover 阴影效果
- 嵌入图片：居中显示、圆角阴影、hover 微放大
- 搜索高亮：黄色背景标记所有匹配，蓝色高亮当前选中项
...
#### 数学公式（KaTeX）
- 支持行内公式 `$...$` 和块级公式 `$$...$$`
- 使用 KaTeX 高性能渲染，支持所有 LaTeX 数学语法
- 暗色主题自动适配

#### Mermaid 图表
- 支持 Mermaid 语法的流程图、时序图、甘特图、类图等
- 代码块标记为 `mermaid` 语言时自动渲染为 SVG 图表
- 暗色主题自动切换

#### 代码块增强
- **复制按钮**：每个代码块右上角显示复制按钮，点击一键复制代码
- **行号显示**：代码块左侧显示行号，便于引用和讨论
- **语言标签**：左上角显示代码语言标识

#### 图片 Lightbox
- 点击文档中的图片可放大查看（Lightbox 效果）
- 全屏遮罩层 + 居中显示，点击任意位置关闭
- 支持大图自适应屏幕尺寸

#### Obsidian 特殊语法
- **Callout 块**：支持 `> [!note]`、`> [!warning]`、`> [!tip]` 等语法，带图标和颜色区分
- **Wikilink**：支持 `[[页面名]]` 和 `[[页面名|显示文本]]` 语法，点击可跳转到对应文档

### 🔧 技术细节
- 新增 CDN 依赖：KaTeX 0.16.11、Mermaid 11
- 自定义 marked.js renderer 处理代码块渲染
- 数学公式预处理保护机制，避免被 Markdown 解析器干扰
- Mermaid 渲染失败时显示原始代码而非报错

## [3.6.0] - 2026-06-19

### 📊 同步进度条
- **实时进度条**：同步过程中在顶栏下方显示进度条（SSE Server-Sent Events 推送）
- 进度条显示当前阶段（扫描/获取/处理/同步）、已处理/总数、进度百分比
- 支持三种同步类型：FNS→暂存、暂存→知识库、暂存→FNS
- 同步完成后自动隐藏，支持手动关闭

### 📋 同步历史日志
- **同步历史记录**：每次同步自动记录类型、状态、文件数、耗时等信息
- **历史弹窗**：顶栏新增「📋 历史」按钮，点击查看最近 50 条同步记录
- 记录包含：同步类型、成功/失败状态、新增/更新/未变/跳过/错误/删除数、耗时
- 历史持久化存储，重启不丢失

### 🔧 后端
- StagingManager 新增 `_sync_history.json` 持久化同步历史
- 三个 sync 方法新增 `progress_callback` 参数，支持进度回调
- SyncEngine 新增 `_progress_queues` 管理 SSE 连接
- 新增 `GET /api/sync/progress?type=fns|kb|to_fns` SSE 端点
- 新增 `GET /api/sync/history` 端点
- WebUI 路由新增 2 条（共 12 条）
